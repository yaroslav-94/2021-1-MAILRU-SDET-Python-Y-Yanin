...
from code_hw.mock import flask_mock
from code_hw.socket_client.client import SocketClient
...
def start_app(config):
    ...
    env = copy(os.environ)
    env['APP_HOST'] = settings_data.APP_HOST
    env['APP_PORT'] = settings_data.APP_PORT

    env['STUB_HOST'] = settings_data.STUB_HOST
    env['STUB_PORT'] = settings_data.STUB_PORT

    env['MOCK_HOST'] = settings_data.MOCK_HOST
    env['MOCK_PORT'] = settings_data.MOCK_PORT
    ...
    if __start_waiter(host=settings_data.APP_HOST, port=settings_data.APP_PORT):
        app_out.writelines(f"App on url '{settings_data.APP_HOST}:{settings_data.APP_PORT}' start successfull!")
...
def start_stub(config):
    ...
    if __start_waiter(host=settings_data.STUB_HOST, port=settings_data.STUB_PORT):
        stub_out.writelines(f"Stub on url '{settings_data.STUB_HOST}:{settings_data.STUB_PORT}' start successfull!")
...
def start_mock(config):
    ...
    env = copy(os.environ)
    env['STUB_HOST'] = settings_data.MOCK_HOST
    env['STUB_PORT'] = settings_data.MOCK_PORT

    flask_mock.ActionWithMock().run_mock()
    if __start_waiter(host=settings_data.MOCK_HOST, port=settings_data.MOCK_PORT):
        mock_out.writelines(f"Mock on url '{settings_data.MOCK_HOST}:{settings_data.MOCK_PORT}' start successfull!")
...
def stop_mock(config):
    requests.get(f'http://{settings_data.MOCK_HOST}:{settings_data.MOCK_PORT}/shutdown')
